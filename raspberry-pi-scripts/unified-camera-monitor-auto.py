...
- YOLOv26 Integration für Vogel-Erkennung
- Automatische Aufnahme bei Trigger
- Cooldown-Management
- System-Monitoring

Verwendung:
    python3 unified-camera-monitor-auto.py --threshold 0.4 --cooldown 15
"""
...
class UnifiedCameraMonitor:
    ...
    def __init__(
        ...
        self.executor = ThreadPoolExecutor(max_workers=3)  # Für parallele Video+Audio+Conversion
    ...
    def _load_model(self) -> bool:
        """Lädt YOLO26-Model für Vogel-Erkennung."""
        if not HAS_YOLO:
            logger.error("YOLO nicht verfügbar")
            return False
        
        try:
            # Verwende YOLO26n für Performance auf Raspberry Pi
    ...
    def stop(self):
        ...
        # Stoppe Audio-Recording falls noch aktiv
        self._stop_audio_recording()
        
        # Beende ThreadPoolExecutor
        if self.executor:
    ...
    def _start_recording(self) -> Optional[str]:
        ...
        with self.recording_lock:
            ...
            try:
                ...
                # PARALLEL TASKS mit ThreadPoolExecutor:
                # 1. Fortschrittsbalken + Video-Aufnahme
                # 2. Audio-Aufnahme in separatem Thread
                # 3. Nach Video-Ende: H264→MP4 + Audio-Merge
                ...
                def recording_with_progress():
                    ...
                    while time.time() - start_time < duration:
                        ...
                        print(f"\r🎥 Aufnahme läuft... {bar} {percent}% ({int(elapsed)}/{duration}s) [Video+Audio parallel]", end='', flush=True)
                        time.sleep(1)
                    
                    print()  # Neue Zeile nach Fortschrittsbalken
                    self._stop_recording()
                    self._stop_audio_recording()  # Stoppe parallel laufendes Audio
                
                # Starte PARALLEL:
                # 1. Video-Aufnahme mit Fortschrittsbalken
                self.executor.submit(recording_with_progress)
                
                # 2. Audio-Aufnahme in separatem Thread (startet SOFORT, läuft parallel zu Video)
                self.executor.submit(self._start_audio_recording, video_dir)
                
                # 3. Nach Video-Ende: Konvertierung + Audio-Merge (optimiert mit overlap)
                def convert_and_merge_after_recording():
                    try:
                        time.sleep(self.recording_duration + 1)
                        
                        # Prüfe ob Audio vorhanden ist (mit kurzer Wartezeit)
                        audio_available = False
                        for _ in range(10):  # Max 5 Sekunden Wartezeit
                        ...
                        if audio_available:
                            logger.info("🔄 Audio verfügbar - starte Merge...")
                            self._convert_h264_to_mp4_with_audio(video_file, video_dir)
                        else:
                            logger.info("⚠️  Audio nicht verfügbar - Fallback zu Video-Only Konvertierung")
                            self._convert_h264_to_mp4_fallback(video_file)
                    
                    except Exception as e:
                        logger.error(f"❌ KRITISCHER FEHLER in Konvertierung: {e}")
                        import traceback
                        logger.error(traceback.format_exc())
                        # Versuche Video-Only Fallback
                        try:
                            logger.info("🔄 Versuche Video-Only Fallback...")
                            self._convert_h264_to_mp4_fallback(video_file)
                        except Exception as e2:
                            logger.error(f"❌ AUCH FALLBACK gescheitert: {e2}")
                
                # Submit mit Exception-Handling
                future = self.executor.submit(convert_and_merge_after_recording)
                # Callback für Fehler
                def handle_conversion_error(fut):
                    try:
                        result = fut.result()  # Prüfe auf Exceptions
                        logger.info("✅ Konvertierungs-Task erfolgreich abgeschlossen")
                        # Flush logs nach erfolgreicher Konvertierung
                        for handler in logger.handlers:
                            if hasattr(handler, 'flush'):
                                handler.flush()
                    except Exception as e:
                        logger.error(f"❌ Unerwarteter Fehler in Konvertierungs-Thread: {e}")
                        import traceback
                        logger.error(traceback.format_exc())
                        # Flush auch bei Fehler
                        for handler in logger.handlers:
                            if hasattr(handler, 'flush'):
                                handler.flush()
                future.add_done_callback(handle_conversion_error)
    ...
    def _stop_recording(self):
        ...
        with self.recording_lock:
            ...
            try:
                ...
                # WICHTIG: Flush nach kritischer Operation
                for handler in logger.handlers:
                    if hasattr(handler, 'flush'):
                        handler.flush()
    ...
    def _start_audio_recording(self, video_dir: Path):
        """Startet Audio-Aufnahme PARALLEL zur Video-Aufnahme mit ffmpeg/arecord."""
        try:
            base_name = datetime.now().strftime("%H-%M-%S")
            self.audio_file = video_dir / f"audio_{base_name}.wav"
            
            logger.info(f"🎤 Starte Audio-Aufnahme: {self.audio_file.name}")
            
            # Versuche ffmpeg ALSA Input (mit Timeout)
            try:
                self.audio_process = subprocess.Popen(
                    ['ffmpeg', '-f', 'alsa', '-i', 'default', '-t', str(self.recording_duration),
                     '-q:a', '9', '-acodec', 'libmp3lame', str(self.audio_file)],
                    stdout=subprocess.DEVNULL,
                    stderr=subprocess.DEVNULL,
                    timeout=self.recording_duration + 10
                )
                
                # Warte mit Timeout
                self.audio_process.wait(timeout=self.recording_duration + 10)
                logger.info(f"✅ Audio-Aufnahme erfolgreich: {self.audio_file.name}")
                return
                
            except FileNotFoundError:
                logger.info("⚠️  ffmpeg nicht verfügbar, versuche arecord...")
            except subprocess.TimeoutExpired:
                logger.warning("⚠️  ffmpeg Timeout - stoppe Audio")
                try:
                ...
                logger.warning(f"⚠️  ffmpeg Fehler: {e}")
            
            # Fallback: arecord
            try:
                self.audio_process = subprocess.Popen(
                    ['arecord', '-d', str(self.recording_duration), '-f', 'S16_LE',
                     '-r', '44100', '-c', '2', str(self.audio_file)],
                    stdout=subprocess.DEVNULL,
                    stderr=subprocess.DEVNULL,
                    timeout=self.recording_duration + 10
                )
                ...
                self.audio_process.wait(timeout=self.recording_duration + 10)
                logger.info(f"✅ Audio mit arecord erfolgreich: {self.audio_file.name}")
                return
                
            except FileNotFoundError:
                logger.warning("⚠️  arecord auch nicht verfügbar")
            except subprocess.TimeoutExpired:
                logger.warning("⚠️  arecord Timeout - stoppe Audio")
                try:
                    self.audio_process.kill()
                except:
                    pass
            except Exception as e:
                logger.warning(f"⚠️  arecord Fehler: {e}")
            
            # Keine Audio-Tools verfügbar
            logger.warning("⚠️  Keine Audio-Tools verfügbar - fahre ohne Audio fort")
            self.audio_file = None
            self.audio_process = None
            
        except Exception as e:
            logger.error(f"❌ Unerwarteter Fehler in Audio-Recording: {e}")
            self.audio_file = None
            self.audio_process = None
    
    def _stop_audio_recording(self):
        """Stoppt laufende Audio-Aufnahme (falls noch aktiv)."""
    ...
    def _convert_h264_to_mp4_fallback(self, h264_file: Path):
        """Fallback: Konvertiert H264→MP4 OHNE Audio (garantiert stabil)."""
        try:
            logger.info(f"🔄 Fallback-Konvertierung (Video-Only): {h264_file.name}")
            
            import subprocess
            
            # Bestimme Playback-FPS (für Zeitlupe)
            if self.recording_fps >= 100:
                playback_fps_list = [5, 10, 20, 30, 120]
                logger.info(f"🎬 Zeitlupen-Konvertierung mit {len(playback_fps_list)} Frameraten")
            else:
                playback_fps_list = [self.recording_fps]
                logger.info(f"📹 Standard-Konvertierung")
            ...
            for playback_fps in playback_fps_list:
                ...
                try:
                    ...
                    if result.returncode == 0:
                        logger.info(f"✅ {mp4_file.name} erstellt ({playback_fps}fps, Video-Only)")
                        success_count += 1
                    else:
                        logger.warning(f"⚠️  ffmpeg returned {result.returncode} für {playback_fps}fps")
                
                except subprocess.TimeoutExpired:
                    logger.error(f"❌ ffmpeg TIMEOUT bei {playback_fps}fps - wahrscheinlich große Datei")
                except FileNotFoundError:
                    logger.error(f"❌ ffmpeg nicht installiert!")
                    return
                except Exception as e:
                    logger.error(f"❌ Fehler bei {playback_fps}fps: {e}")
            
            # Cleanup: Lösche H264 nur wenn erfolgreich konvertiert
            if success_count > 0:
                h264_file.unlink()
                logger.info(f"✅ Fallback-Konvertierung erfolgreich: {success_count} MP4-Dateien erstellt")
            else:
                logger.error(f"❌ KEINE erfolgreichen Konvertierungen - H264 bleibt erhalten!")
                
        except Exception as e:
            logger.error(f"❌ KRITISCHER FEHLER in Fallback-Konvertierung: {e}")
            import traceback
            logger.error(traceback.format_exc())
    
    def _convert_h264_to_mp4_with_audio(self, h264_file: Path, video_dir: Path):
        """Konvertiert H264→MP4 und merged mit Audio (falls vorhanden)."""
        try:
            import subprocess
            
            # Warte kurz bis Audio-Datei geschrieben ist
            max_wait = 5
            waited = 0
            while not self.audio_file or not self.audio_file.exists():
                if waited > max_wait:
                    logger.warning(f"⚠️  Audio-Datei nicht gefunden nach {max_wait}s - fahre ohne Audio fort")
                    self.audio_file = None
                    break
                time.sleep(0.5)
                waited += 0.5
            ...
            if self.recording_fps >= 100:
                playback_fps_list = [5, 10, 20, 30, 120]
                logger.info(f"🔄 Konvertiere Zeitlupen-Video mit Audio ({len(playback_fps_list)} Frameraten)...")
            else:
                playback_fps_list = [self.recording_fps]
                logger.info(f"🔄 Konvertiere Video mit Audio zu MP4...")
            ...
            for playback_fps in playback_fps_list:
                ...
                if self.audio_file and self.audio_file.exists():
                    # MERGE: H264 Video + Audio zu MP4
                    logger.info(f"🎬 Merge Video + Audio für {playback_fps}fps...")
                    result = subprocess.run(
                        ['ffmpeg', '-fflags', '+genpts', '-r', str(playback_fps),
                         '-i', str(h264_file), '-i', str(self.audio_file),
                         '-c:v', 'copy', '-c:a', 'aac', '-af', 'volume=4.0,loudnorm=I=-23', '-shortest', '-y', str(mp4_file)],
                        capture_output=True,
                        text=True,
                        timeout=300
                    )
                else:
                    # OHNE Audio: Nur Video konvertieren
                    logger.info(f"🎬 Konvertiere Video zu {playback_fps}fps (ohne Audio)...")
                    result = subprocess.run(
                        ['ffmpeg', '-fflags', '+genpts', '-r', str(playback_fps),
                         '-i', str(h264_file), '-c:v', 'copy', '-y', str(mp4_file)],
                        capture_output=True,
            ...
            # Cleanup: Lösche H264 und Audio-Dateien
            if success_count > 0:
                h264_file.unlink()
                ...
                if self.audio_file and self.audio_file.exists():
                    ...
                    logger.info(f"🗑️  Audio-Datei gelöscht: {self.audio_file.name}")
                
                logger.info(f"✅ Konvertierung abgeschlossen: {success_count} MP4-Dateien mit Audio erstellt")
                self.audio_file = None
            else:
                logger.error(f"❌ Keine erfolgreichen Konvertierungen")
                
        except Exception as e:
            logger.error(f"❌ Fehler bei Konvertierung/Merge: {e}")
    ...
    def _convert_h264_to_mp4(self, h264_file: Path):
        ...
        try:
            ...
            # WICHTIG: Flush logger nach erfolgreicher Konvertierung
            for handler in logger.handlers:
                if hasattr(handler, 'flush'):
                    handler.flush()
                
        except Exception as e:
            logger.error(f"❌ KRITISCHER FEHLER bei Konvertierung: {e}")
            import traceback
            logger.error(traceback.format_exc())
            # Flush auch bei Fehler
            for handler in logger.handlers:
                ...
                try:
                    ...
                    if self._check_trigger(bird_detected):
                        ...
                        if video_file:
                            ...
                            # WICHTIG: Nach Aufnahme-Start Flush
                            for handler in logger.handlers:
                                if hasattr(handler, 'flush'):
                                    handler.flush()
...
def main():
    ...
    parser.add_argument('--enable-audio', action='store_true', help='Audio-Aufnahme aktivieren (parallel zum Video)')
